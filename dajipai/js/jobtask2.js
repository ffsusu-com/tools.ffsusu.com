function jobtask() {
    ...
    $(function () {
        var csvList;
        $(".link").empty();
        $(".link").append('<div id="type2num" class="type2num"></div><div id="type2num_2" class="type2num"></div><div id="type2num_3" class="type2num"></div><ul id="jobtask"></ul>');
        $(".explain").empty();
        var insert2 = '';
        insert2 += '<p class="explain-title">使用说明</p><ul style="height:130px;overflow:auto;">';
        insert2 += '<p class="explain-main">因1-30任务皆在各职业公会处接取，所以1-30不做任务引导</p>';
        insert2 += '<p class="explain-main">因60-70的职业任务是由NPC提供材料做任务道具，所以也不会有缴纳物品提供</p>';        
        $.ajax({
            url: './csv/jobtask.csv',
...
function jobtaskexplain(obj, i) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            insert += '<img style="float:left;" src="tupian/jobtask/' + csvList[i][0] + '.png">';                        
