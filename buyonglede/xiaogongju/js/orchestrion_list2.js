    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 1; i < csvList.length; i++) {
                ...
                if ( isset(csvList[i][7]) ) {
                    insert += '<td class="checkbox" rowspan="3"><a onclick="listen(' + csvList[i][10] + ')" target="_blank">' + csvList[i][1] + '</a></td>'
                }else{
                    insert += '<td class="checkbox"><a onclick="listen(' + csvList[i][10] + ')" target="_blank">' + csvList[i][1] + '</a></td>'
                }
                if (csvList[i][2] == 0) {
...
function saveMusicdata(){
    ...
    //配列をクッキー保存
    $.cookie("musicData",musicArray, { expires: 365 , path: "/" });
}

function listen(i) {
    var music = document.getElementById("BGML");
    music.src = "//music.163.com/outchain/player?type=2&id=" + i + "&auto=1&height=32";
    //var idstart = music.src.indexOf("id=");
    //var idend = music.src.indexOf("&auto");
    //var id = music.src.substring(idstart + 3, idend);
    //if (id == i)
    //{ music.src = "//music.163.com/outchain/player?type=2&id=" + i + "&auto=0&height=32"; }
    //else { music.src = "//music.163.com/outchain/player?type=2&id=" + i + "&auto=1&height=32"; }
}