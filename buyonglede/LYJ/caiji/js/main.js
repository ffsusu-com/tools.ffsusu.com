...
function checkScroll() {
	...
	$("#posbase_footer").css({"width": "90%", "position": "absolute", "top": $("#posbase_comment").offset().top + $("#posbase_comment").height() + 20, "left": 0 });
	...
	for (var i in pointData) {
		...
		if (isSMP) {
			...
			html += sprintf(template, [
				...
				("園芸師" == data["type"] ? "image/smp/icon_btn_24.png" : "image/smp/icon_min_24.png"),
...
function updateItemFilter() {
	...
	for (var i in pointDataForFilter) {
		...
		while (1) {
			//クラス
			if ((1 == filterClass && "園芸師" != data["type"]) ||
				(2 == filterClass && "採掘師" != data["type"])) {
				break;
			}
...
function onClickLine(inputId, baseObject) {
	...
	*/
	...
	if (!isSMP) {
		...
		$("#map_mark").attr("src", "image/sys/mark_" + ("採掘師" == data["type"] ? "min" : "btn") + ".png");
